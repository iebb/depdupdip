...
if(isset($_GET['search'])&&trim($_GET['search'])!=""){
    ...
    $filter_sql=" ( title like '%$search%' or source like '%$search%' or problem_id like '%$search%' or description like '%$search%')"; // 
...
while ($row=mysql_fetch_object($result)){
	...
	if (isset($sub_arr[$row->problem_id])){
		if (isset($acc_arr[$row->problem_id])) 
			$view_problemset[$i][0]="<b><a class='nounderline' href='status.php?problem_id=".$row->problem_id ."&user_id=". $_SESSION['user_id']."'><span class='label label-success'>Accepted</span></a></b>";
		else 
			$view_problemset[$i][0]= "<b><a class='nounderline' href='status.php?problem_id=". $row->problem_id ."&user_id=". $_SESSION['user_id']."'><span class='label label-important'>Unaccepted</span></a></b>";
	}else{
		$view_problemset[$i][0]= "<b><a class='nounderline'><span class='label label-inverse'>Not Submitted</span></a></b>";
	}
	$view_problemset[$i][1]="<span class='center'>".$row->problem_id."</span>&nbsp;".file_get_contents("http://127.0.0.1/showacuseravt.php?id=".$row->problem_id);
