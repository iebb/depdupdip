<?php
	...
	if(isset( $_GET ['start'] ))
		...
		while ( $row = mysql_fetch_object ( $result ) ) {
			...
			$view_rank[$i][1]=  "<div class=left style='margin: 0px 0px 9px; font-family: Ubuntu; font-size: 30px; line-height: 18px;'>".file_get_contents("http://127.0.0.1/ranklistavatar.php?user=".$row->user_id."&rank=".$rank)."<a href='userinfo.php?user=" . $row->user_id . "'>" . $row->user_id . "</a>" ."</div>";
			}
			else if ($rank<=10)
			{
			...
			$view_rank[$i][1]=  "<div class=left style='margin: 0px 0px 9px; font-family: Ubuntu; font-size: 20px; line-height: 18px;'>".file_get_contents("http://127.0.0.1/ranklistavatar.php?user=".$row->user_id."&rank=".$rank)."<a href='userinfo.php?user=" . $row->user_id . "'>" . $row->user_id . "</a>" ."</div>";
			}
			else
			{
				$view_rank[$i][0]= $rank;
				$view_rank[$i][1]=  "<div class=left>".file_get_contents("http://127.0.0.1/ranklistavatar.php?user=".$row->user_id."&rank=".$rank)."<a href='userinfo.php?user=" . $row->user_id . "'>" . $row->user_id . "</a>" ."</div>";	}
